...
function getSumBetweenNumbers(n1, n2) {
  return Array(n2 - n1 + 1).fill(n1).reduce((acc, item, index) => acc + item + index, 0);
}
...
function findFirstSingleChar(str) {
  const result = [...str].reduce((acc, item, index) => {
    if (!acc.length) {
      if ((str.slice(0, index) + str.slice(index + 1)).indexOf(item) === -1) {
        acc.push(item);
      }
    }
    return acc;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return a < b
    ? `${bracketStart}${a}, ${b}${bracketEnd}`
    : `${bracketStart}${b}, ${a}${bracketEnd}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = arr.reduce((acc, item, index) => {
    if (arr.length % 2 === 0 ? index % 2 === 0 : (index + 1) % 2 === 0) {
      if (item * 2 > 9) {
        return acc + (item * 2 - 9);
      }
      return acc + item * 2;
...
function getDigitalRoot(num) {
  const result = [...num.toString()].reduce((acc, item) => acc + +item, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const bracket = str[i];
    const openCloseBrackets = bracketsConfig.find((br) => bracket === br[0] || bracket === br[1]);
    if (!stack.length && bracket === openCloseBrackets[1]) return false;
    if (stack.length && bracket === openCloseBrackets[1]) {
      if (openCloseBrackets[0] === stack[stack.length - 1]) {
        stack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    ...
    const checkPathes = pathes.every((path) => path[i] === char);
    if (!checkPathes) {
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m2.length; i += 1) {
    ...
    let counter = 0;
    const char = m2[i];
    if (char.length === m1[0].length) {
      while (counter < char.length) {
        let value = 0;
        for (let j = 0; j < char.length; j += 1) {
          value += (char[counter] * m1[i][j]);
        }
      ...
      someResult += (char[counter] * m1[0][i]);
    }
  }
  if (!stack.length) {
    stack.push([someResult]);
  }
  return stack;
}
